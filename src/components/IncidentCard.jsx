...
const IncidentCard = ({ incident, onClick }) => {
  ...
  return (
    <div className="bg-white rounded-lg border p-4 hover:shadow-md transition cursor-pointer" onClick={onClick}>
      <div className="flex justify-between items-start mb-3">
        ...
        <span className={`px-3 py-1 rounded-full text-xs font-medium border ${severityColors[incident.severity]}`}>{incident.severity}</span>
      </div>
      ...
      <div className="mt-3 pt-3 border-t flex items-center justify-between text-xs">
        <p className="text-gray-500">Assigned to: <span className="font-medium text-gray-700">{incident.assignedTo}</span></p>
